...
def main (args):
    ...
    accepted_labels = [ "NO RELATIONSHIP ASSERTED",
                        "TOWARD(got there)",
                        "FROM",
                        "NEAR",
                        "IN",
    ...
    df = df[df["Spatial Relation"].isin (accepted_labels)]
    train_df, test_df = train_test_split(df, test_size=0.2, random_state=96)

    bertRE = BERTRelationPrediction (model_name=args.pretrained_model_name, bert_dims=768, n_labels=8)
    bertRE.to(device)
